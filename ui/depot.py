...
from controllers.client_controller import deposit_customer_account
...
class DepotWindow(QWidget):
    ...
    def __init__(self, parent):
        super().__init__()
        self.setWindowTitle("Page de depot")
    ...
    def init_ui(self):
        ...
        title = QLabel()
        title.setText('Operation de depot')
    ...
    def depot_compte(self):
        transaction_solde = deposit_customer_account(self.parentWindow.user_identifiant, self.pin.text(),int(self.amount.text()))
        if(transaction_solde.login_done and transaction_solde.depot_done):
            QMessageBox.information(self, "Transaction effectué",transaction_solde.message_depot,buttons=QMessageBox.StandardButton.Ok,defaultButton=QMessageBox.StandardButton.Ok)
        else:
